...
abstract class BasePaginationCubit<T, S extends BasePaginationState<T>>
    extends Cubit<S> {
  BasePaginationCubit(super.initialState);

  // A method that must be implemented in the inheriting Cubit to call its UseCase
  Future<Either<Failure, PaginatedDataResponse<T>>> getUseCaseCall(
  ...
  Future<void> _fetchData({required bool isInitial}) async {
    ...
    response.fold(
      ...
      (data) {
        final List<T> updatedList = isInitial
            ? data.list
